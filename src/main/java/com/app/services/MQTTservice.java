...
import com.app.activity.TopicDescriptionActivity;
...
public class MQTTservice extends Service {
    ...
    double publisherLatitude, publisherLongitude, subscriberLatitude, subscriberLongitude;
    String publisherLongDescription, userName;
    ...
    public void onCreate()
    {
        ...
        clientMessenger = new Messenger(connection.msgHandler);
    }
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    ...
                    case SUBSCRIBE: {
                        ...
                        Bundle b = msg.getData();
                        if (b != null) {
                            subscriberLatitude = msg.getData().getDouble("subscriberLatitude");
                            subscriberLongitude = msg.getData().getDouble("subscriberLongitude");
                    ...
                    case PUBLISH: {
                        ...
                        Bundle b = msg.getData();
                        if (b != null) {
                            publisherLongDescription = msg.getData().getString("longDescription");
                            publisherLatitude = msg.getData().getDouble("publisherLatitude");
                            publisherLongitude = msg.getData().getDouble("publisherLongitude");
                            /*userName = msg.getData().getString("userName");*/
                            StringBuilder message= new StringBuilder();
                           // message.append(b.getCharSequence(LONGDESC+","));
                            CharSequence cs = b.getCharSequence(USERNAME);
                            if (cs != null) {
                                userName = cs.toString().trim();
                                if (!userName.isEmpty()) {
                                    message.append(userName+",");
                                    cs = b.getCharSequence(TOPIC);
                                    if (cs != null) {
                                        String topic = cs.toString().trim();

                                        if (!topic.isEmpty()) {
                                            message.append(topic+",");
                                            cs = b.getCharSequence(SHORTDESC);
                                            if (cs != null) {
                                                String shortDesc = cs.toString().trim();
                                                if (!shortDesc.isEmpty()) {
                                                    message.append(shortDesc+",");
                                                    cs = b.getCharSequence(LONGDESC);
                                                    if(cs!=null){
                                                    ...
                                                    status = publish(topic, message.toString());
                                                }
                                            }
                                        }
                                    }
...
*/
                ...
                Location publisherLocation = new Location("");
                publisherLocation.setLatitude(publisherLatitude);
                publisherLocation.setLongitude(publisherLongitude);

                Location subscriberLocation = new Location("");
                subscriberLocation.setLatitude(subscriberLatitude);
                subscriberLocation.setLongitude(subscriberLongitude);

                float distance = publisherLocation.distanceTo(subscriberLocation);

                if (distance <= 20000000.0F) {
                    ...
                    try {
                        Intent intent = new Intent(MQTTservice.this, TopicDescriptionActivity.class);
                        ...
                        notification.setLatestEventInfo(context, topic, message.toString(), pendingIntent);
